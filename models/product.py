...
class Product:
    ...
    def set_product_name(self,productname):
        'validate product name before setting (2-30 alphabet/underscore)'
        ...
        while True:
            if pattern.match(productname):
                ...
                print("\t\t Invalid product name, must have only alphabets &_!!!!")
                produntname = input('\t\tEnter Product Name again: ')
    ...
    # override __str__
    def __str__(self):
        return f'Produnt ID :{self.__product_id}, Product Name :{self.__productname}, Category ID :{self.__category_id}, Unit price :{self.__unitprice}, Manufacture Date :{self.__manufacture_date}, Is Active :{self.__is_active}'